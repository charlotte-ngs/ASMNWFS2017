...
- Für unser Regressionsmodell
    + Annahme: $\sigma^2$ sei bekannt und ist im Folgenden nicht mehr erwähnt
    + a posteriori Verteilung für $\beta$: $f(\beta | \mathbf{y})$
- Berechnung durch __Satz von Bayes__, dieser basiert auf der Definition der bedingten Wahrscheinlichkeit

\begin{eqnarray}
f(\mathbf{\beta} | \mathbf{y}) & = & \frac{f(\mathbf{\beta}, \mathbf{y})}{f(\mathbf{y})} \nonumber \\
                               & = & \frac{f(\mathbf{y} | \mathbf{\beta})f(\mathbf{\beta})}{f(\mathbf{y})}
...
- $f(\mathbf{y} | \mathbf{\beta})$: Likelihood
- $f(\mathbf{\beta})$: a priori Verteilung von $\beta$
...
- A Posteriori Verteilung für Regression: $f(\mathbf{\beta} | \mathbf{y})$
- Vollbedingte Verteilungen für Regression: 
    + $f(\beta_0 | \beta_1, \mathbf{y})$
    + $f(\beta_1 | \beta_0, \mathbf{y})$
