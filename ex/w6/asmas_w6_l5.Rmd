...
Das Modell (\ref{eq:GBlupModel}) ist ein gemischtes lineares Modell mit den zufälligen Effekten $g$. Die Varianz $Var(g) = G \sigma_g^2$ wobei $G$ der __genomischen Verwandtschaftsmatrix__ entspricht und $\sigma_g^2$ der genetisch additiven Varianz entspricht. Die genomische Verwandtschaftsmatrix $G$ baisert auf SNP-Informationen der typisierten Individuen und kann auf verschiedene Arten berechnet werden. 
