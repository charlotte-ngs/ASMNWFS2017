...
```{r TukeyAnscombPlot, fig.show=TRUE}
...
wobei  &              &  \\
       &  $y$         &  Vektor der Länge $n$ mit phänotypischen Beobachtungen \\
       &  $\mu$       &  allgemeines Mittel \\
       &  $u$         &  Vektor der Länge $n$ mit zufälligen additiven SNP-Effekten pro Individuum \\
       &  $Z$         &  Inzidenzmatrix, welche SNP-Effekte mit Beobachtungen verknüpft\\
       &  $\epsilon$  &  Vektor von zufälligen Resteffekten
\end{tabular}

Das Modell (\ref{eq:GBlupModel}) ist ein gemischtes lineares Modell mit den zufälligen Effekten $u$. Die Varianz $Var(u) = G \sigma_u^2$ wobei $G$ der __genomischen Verwandtschaftsmatrix__ entspricht und $\sigma_u^2$ der genetisch additiven Varianz entspricht. 
