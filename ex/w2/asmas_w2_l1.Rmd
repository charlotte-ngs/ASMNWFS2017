...
In einem kleinen Beispieldatensatz sind die SNP-Genotypen für `r nAnzTiere` Tiere gegeben. Für jedes Tier liegen Typisierungsergebnisse an `r nAnzSnp` SNPs vor. Die Bezeichnung $(G_kG_l)_{ij}$ steht für den Genotypen für Tier $i$ an der SNP-Position $j$ mit den Allelen $k$ und $l$. Da wir nur SNPs mit zwei Allelen betrachten können als $k$ und $l$ nur entweder $0$ oder $1$ sein. Wir nehmen auch an, dass das Allel $0$ immer das Allel mit der gewünschten Ausprägung ist. In der folgenden Tabelle sind die SNP-Genotypen für alle Tiere aufgelistet, wobei die Indices $i$ und $j$ weggelassen wurden. 
