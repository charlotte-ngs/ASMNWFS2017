...
\begin{enumerate}
\item[c)] Gegeben ist der folgende Datensatz. Bei den SNPs gilt $G_1$ als Allel mit der positiven Wirkung. Stellen Sie die Modelle als Formeln und die Gleichungssysteme für RR-BLUP und GBLUP auf. Verwenden Sie bei den Gleichungssystem so weit als möglich die im Datensatz gegebenen Zahlenwerte. Als fixen Effekt können Sie ein allgemeines Mittel $\mu$ annehmen. Das Verhältnis zwischen Restvarianz und genetischer Varianz $\lambda$ betrage $\lambda = 1$.  
\end{enumerate}
